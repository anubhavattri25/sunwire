...
async function renderAdminPage(req, res) {
  ...
  const runtimeScript = [
    ...
    '</script>',
    '<script type="module" src="/admin/news.js?v=20260401-6"></script>',
  ].join('');
  ...
  if (req.method === 'PATCH') {
    ...
    if (action === 'story-signals') {
      ...
      const liveUpdateItems = Array.isArray(body?.liveUpdates?.items)
        ...
        : [];
      const invalidLiveUpdate = liveUpdateItems.find((item) => countWords(String(item?.text || item || '')) > 20);
      if (invalidLiveUpdate) {
        return res.status(400).json({ error: 'Each live update line must stay within 20 words.' });
      }
