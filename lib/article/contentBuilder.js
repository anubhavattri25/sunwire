...
function buildMetaDescription({ title = "", summary = "", deepDive = [], indiaPulse = "" } = {}) {
  ...
  return trimToLength(candidate || title, 160);
}
...
function buildKeyPointNarrative(keyPoints = []) {
  const items = Array.isArray(keyPoints)
    ? keyPoints.map((entry) => sentenceToClause(entry)).filter(Boolean)
    : [];
  if (!items.length) return "";
  if (items.length === 1) return `${items[0]}.`;
  if (items.length === 2) return `${items[0]}, while ${items[1].charAt(0).toLowerCase()}${items[1].slice(1)}.`;
  return `${items.slice(0, -1).join(", ")}, and ${items[items.length - 1].charAt(0).toLowerCase()}${items[items.length - 1].slice(1)}.`;
}

function buildWhyItMattersParagraph({ title = "", category = "", tags = [], primarySource = "" } = {}) {
  const section = normalizeCategoryLabel(category);
  const tagText = tags.length
    ? `Key themes around ${tags.slice(0, 3).map(humanizeTag).join(", ")} are driving why this ${section.toLowerCase()} story matters beyond the first headline.`
    : `This ${section.toLowerCase()} update matters because it changes the practical picture behind the headline and the timeline readers should track next.`;
  const sourceText = primarySource
    ? `The primary-source trail from ${primarySource} is especially important because it gives the clearest signal on what has actually been confirmed so far.`
    : "The most reliable signal remains the verified source material rather than recycled social summaries.";
  return cleanText(`${trimToLength(title, 120)}. ${tagText} ${sourceText}`);
}

function buildFactSheetNarrative(rows = []) {
  const items = Array.isArray(rows)
    ? rows.filter((row) => cleanText(row.label || "") && cleanText(row.value || ""))
    : [];
  if (!items.length) return "";
  return cleanText(
    items
      .slice(0, 4)
      .map((row) => `${cleanText(row.label)}: ${cleanText(row.value)}`)
      .join(". ")
  );
}

function buildWhatNextParagraph({ source = "", publishedAt = "", tags = [] } = {}) {
  const timing = formatPublishedLabel(publishedAt);
  const sourceLabel = cleanText(source || "the primary source");
  const tagText = tags.length
    ? `Watch for the next verified movement around ${tags.slice(0, 3).map(humanizeTag).join(", ")}.`
    : "Watch for the next verified update rather than speculative reposts.";
  return cleanText(`As of ${timing}, ${sourceLabel} remains the clearest reference point for the next concrete development. ${tagText}`);
}

function buildLongformBody({
  title = "",
  category = "",
  summary = "",
  keyPoints = [],
  ...
  const paragraphs = dedupeBy([
    cleanText(summary),
    buildKeyPointNarrative(keyPoints),
    ...
    cleanText(indiaPulse),
    ...background.map((item) => `${item.title}. ${item.context}`),
    buildFactSheetNarrative(factSheet),
    buildWhatNextParagraph({
      source: primarySource?.name || "",
      publishedAt,
      tags,
    }),
  ].map((entry) => cleanText(entry)).filter(Boolean), (entry) => entry.toLowerCase());
