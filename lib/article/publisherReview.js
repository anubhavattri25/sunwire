...
function buildPublisherReview(input = {}, options = {}) {
  ...
  if (!manualUpload && keyPoints.length < MIN_KEY_POINTS) reasons.push(`key_points_below_${MIN_KEY_POINTS}`);
  if (!manualUpload && deepDive.length < MIN_DEEP_DIVE) reasons.push(`deep_dive_below_${MIN_DEEP_DIVE}`);
  if (!manualUpload && background.length < MIN_BACKGROUND) reasons.push(`background_items_below_${MIN_BACKGROUND}`);
  if (!manualUpload && factSheet.length < MIN_FACT_SHEET) reasons.push(`fact_sheet_rows_below_${MIN_FACT_SHEET}`);
  if (!manualUpload && !aiRewritten) reasons.push("auto_story_not_ai_rewritten");
  if (!manualUpload && rewriteStatus !== "ai_rewritten") reasons.push("fallback_rewrite_blocked");
  if (!manualUpload && !/^https?:\/\//i.test(sourceUrl)) reasons.push("missing_primary_source_url");
  if (blockedCommunitySource) reasons.push("community_source_blocked");
  if (!manualUpload && sourceBody && sourceOverlapRatio >= MAX_SOURCE_OVERLAP_RATIO) {
    reasons.push(`source_overlap_ratio_${sourceOverlapRatio.toFixed(2)}`);
  }

  return {
    eligibleForPublisherNetwork: reasons.length === 0,
    showInPublicListings: reasons.length === 0,
