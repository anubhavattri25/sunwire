...
function toCompatStory(record = {}) {
  ...
  return {
    ...
    readerPulse: article.readerPulse || metadata.readerPulse || null,
    liveUpdates: article.liveUpdates || metadata.liveUpdates || null,
    syntheticViews: Number(article.syntheticViews || 0),
    activeLiveUpdates: Array.isArray(article.activeLiveUpdates) ? article.activeLiveUpdates : [],
    liveUpdateTimeline: Array.isArray(article.liveUpdateTimeline) ? article.liveUpdateTimeline : [],
    nextLiveUpdateAt: String(article.nextLiveUpdateAt || "").trim(),
    liveUpdateCount: Number(article.liveUpdateCount || 0),
    activeLiveUpdateCount: Number(article.activeLiveUpdateCount || 0),
...
async function queryStories({ page = 1, pageSize = DEFAULT_PAGE_SIZE, filter = "all" } = {}) {
  ...
  try {
    ...
    throw normalizeDatabaseError(error, 'Story query failed.');
  } finally {
    await releasePrisma();
  }
}
...
async function findStoryBySlug({ slug = "", category = "" } = {}) {
  ...
  if (isDatabaseCoolingDown()) return null;
