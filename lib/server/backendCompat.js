...
async function queryStories({ page = 1, pageSize = DEFAULT_PAGE_SIZE, filter = "all" } = {}) {
  try {
    ...
    const stories = records.map(toCompatStory);
...
async function findStoryBySlug({ slug = "", category = "" } = {}) {
  try {
    ...
    if (exact) return toCompatStory(exact);
...
async function findStoryByIdentity({
  ...
  try {
    ...
    if (requestedId) {
      ...
      if (byId) return toCompatStory(byId);
    }
    ...
    if (requestedUrl) {
      ...
      if (byUrl) return toCompatStory(byUrl);
    }
    ...
    if (requestedTitle) {
      ...
      if (byTitle) return toCompatStory(byTitle);
    }
