...
function buildCategoryMap(allStories = []) {
  ...
  return categories.reduce((acc, category) => {
    acc[category] = dedupeStories(allStories.filter((story) => storyCategoryKey(story) === category));
    return acc;
  }, {});
}
...
function buildHomeView({
  ...
  if (safeFilter === "all") {
    ...
    const randomMix = fillSectionStories(
      deterministicShuffle(storyPool.filter((story) => !used.has(storyKey(story))), safePage),
      deterministicShuffle(storyPool, safePage + 11),
      8,
      new Set()
    );
    moreSections = [
...
function renderHomeTemplate(template = "", view = {}) {
  ...
  return output.replace(
    ...
    `<script>window.__SUNWIRE_HOME_DATA__=${safeJsonForInlineScript(preloadPayload)};</script><script type="module" src="/app.js?v=20260322-7"></script>`,
  );
}
