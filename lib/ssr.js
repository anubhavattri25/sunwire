...
function storyCategoryKey(story = {}) {
  ...
  if (["general", "ai", "tech", "entertainment", "sports", "business", "politics", "jobs", "food", "headline", "trending"].includes(raw)) {
    return raw;
  }
...
function renderHomeTemplate(template = "", view = {}) {
  ...
  output = replaceBlock(
    output,
    /<div class="main-layout" id="heroSection" aria-live="polite">[\s\S]*?<\/div>\s*<!-- RIGHT: SIDEBAR CARDS -->/i,
    hasFullGrid
      ? [
        '<div class="main-layout" id="heroSection" aria-live="polite" hidden>',
        '<div class="left-hero">',
        '<img id="heroImage" src="https://sunwire.in/social-card.svg" alt="SunWire lead story" loading="eager" decoding="async" fetchpriority="high" width="1600" height="900" />',
        '<div class="hero-overlay"></div>',
        '<div class="hero-content">',
        '<span class="hero-kicker" id="heroDeskChip">Live Desk</span>',
        '<h1 class="hero-title"><a id="headlineOfTheDayLink" href="/" target="_self" rel="noopener noreferrer">Loading headline…</a></h1>',
        '<p class="hero-desc" id="heroSummary">Fresh AI, tech, entertainment, sports, and business stories are loading.</p>',
        '<div class="meta-row"><span class="breaking-badge"><span class="breaking-dot"></span>BREAKING</span> <span class="hero-section__author" id="heroAuthor">SunWire Desk</span> <span class="hero-section__time" id="headlineOfTheDayMeta">Waiting for fresh stories.</span></div>',
        '<a id="heroViewStory" href="/" class="hero-view-btn" target="_self" rel="noopener noreferrer">View Full Story →</a>',
        '</div>',
        '</div>'
      ].join("")
      : [
        '<div class="main-layout" id="heroSection" aria-live="polite">',
        '<div class="left-hero">',
        ...
        '<div class="hero-reactions-chip" id="heroReactionsBlock">',
        `<strong id="heroReactionTitle">${escapeHtml(heroTitle.split(':')[0] || "Global Impact")}</strong>`,
        '<span id="heroReactionCaption">— Reactions Pour In</span>',
        '</div>',
        '</div>',
        "</div>",
        '</div>'
...
;
  ...
  return output.replace(
    ...
    `<script>window.__SUNWIRE_HOME_DATA__=${safeJsonForInlineScript(preloadPayload)};window.__SUNWIRE_GOOGLE_CLIENT_ID__=${safeJsonForInlineScript(clientId)};document.documentElement.dataset.googleClientId=${safeJsonForInlineScript(clientId)};var authButton=document.getElementById('authButton');if(authButton){authButton.dataset.googleClientId=${safeJsonForInlineScript(clientId)};}</script><script type="module" src="/app.js?v=20260324-6"></script>`,
  );
}
...
function renderArticleTemplate(template = "", page = {}) {
  ...
  return output.replace(
    ...
    `<script>window.__SUNWIRE_ARTICLE_DATA__=${safeJsonForInlineScript(preloadPayload)};</script><script type="module" src="/article.js?v=20260324-2"></script>`,
  );
}
