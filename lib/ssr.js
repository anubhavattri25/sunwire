...
const { buildStoryPlaceholderImage, hasRenderableStoryImage } = require("./server/storyImages");
...
function renderRelatedGrid(stories = []) {
  ...
  return stories.map((story) => {
    ...
    return [
      ...
      image
        ? `<a class="related-card__media" href="${escapeHtml(href)}"><img src="${escapeHtml(image)}" alt="${escapeHtml(cleanText(story.title || "Story"))}" loading="lazy" decoding="async" width="1600" height="1000" /></a>`
        : "",
...
function renderArticleTemplate(template = "", page = {}) {
  ...
  return output.replace(
    /<script type="module" src="\/?article\.js\?v=20260319-1"><\/script>/,
    `<script>window.__SUNWIRE_ARTICLE_DATA__=${safeJsonForInlineScript(preloadPayload)};</script><script type="module" src="/article.js?v=20260319-1"></script>`,
  );
}
