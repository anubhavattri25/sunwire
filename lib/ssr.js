...
function renderNewsCard(story = {}, variant = "standard") {
  ...
  const summary = trimSummary(story.summary || "", variant === "compact" ? 110 : 150);
...
function buildHomeView({
  ...
  const filteredPageStories = dedupeStories(pageStories);
  const storyPool = dedupeStories(allStories);
...
function renderHomeTemplate(template = "", view = {}) {
  ...
  return output.replace(
    /<script type="module" src="\/?app\.js\?v=20260313-1"><\/script>/,
    `<script>window.__SUNWIRE_HOME_DATA__=${safeJsonForInlineScript(preloadPayload)};</script><script type="module" src="/app.js?v=20260313-1"></script>`,
  );
}
...
function renderPrimarySource(article = {}, story = {}) {
  ...
  if (!cleanText(primarySource.url || "")) return "";

  return [
    '<section class="article-block article-block--source" id="primarySourceBlock">',
    '<div class="article-block__head"><p class="article-block__eyebrow">Source Trail</p><h2>Primary Source</h2></div>',
    '<div class="primary-source">',
    `<a class="primary-source__link" href="${escapeHtml(primarySource.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(cleanText(primarySource.name || "Original Source"))}</a>`,
...
function renderArticleTemplate(template = "", page = {}) {
  ...
  const articleSummary = trimSummary(
    sanitizeArticleCopy(article.summary || story.summary || "", 3) || "No verified summary available.",
    150
  );
  ...
  const deepDive = Array.isArray(article.deepDive) && article.deepDive.length
    ? article.deepDive
    : splitParagraphs(article.body || article.summary || "").slice(0, 4);
  ...
  output = output.replace(
    ...
    [
      ...
      '<div class="article-block__head"><p class="article-block__eyebrow">Analysis</p><h2>The Deep Dive</h2></div>',
  ...
  return output.replace(
    /<script type="module" src="\/?article\.js\?v=20260313-2"><\/script>/,
    `<script>window.__SUNWIRE_ARTICLE_DATA__=${safeJsonForInlineScript(preloadPayload)};</script><script type="module" src="/article.js?v=20260313-3"></script>`,
  );
}
