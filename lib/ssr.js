...
function storyCategoryKey(story = {}) {
  ...
  if (["general", "ai", "tech", "entertainment", "sports", "business", "politics", "jobs", "food"].includes(raw)) {
    return raw;
  }
...
function renderNewsCard(story = {}, variant = "standard") {
  ...
  return [
    ...
    })} alt="${escapeHtml(imageAlt)}" loading="lazy" decoding="async" />`,
...
function buildCategoryMap(allStories = []) {
  const categories = ["ai", "tech", "entertainment", "sports", "business", "politics", "jobs", "food"];
...
function isHomepageGeneralStory(story = {}) {
  return storyCategoryKey(story) !== "food" && !isFoodStory(story);
}
...
function buildHomeView({
  ...
  if (safePage > 1) {
    ...
    const fullGridStories = fillSectionStories(filteredPageStories, fallbackStories, 32, new Set());
    return {
      hero: null,
      trending: [],
      topSections: [],
  ...
  const trending = selectUniqueStories(
    dedupeStories([
      ...homepagePriorityPool.filter((story) => storyKey(story) !== storyKey(hero)),
      ...sortStoriesForHomepageFocus(homepageFallbackPool.filter((story) => storyKey(story) !== storyKey(hero))),
    ]),
    4,
    used
  );
  ...
  if (safeFilter === "all") {
    topSections = buildHomepageDeskSections(storyPool, categoryMap, sortStoriesByRecency(storyPool));
    const randomMix = fillSectionStories(
      deterministicShuffle(homepagePriorityPool.filter((story) => !used.has(storyKey(story))), safePage),
      deterministicShuffle(homepageFallbackPool.filter((story) => !used.has(storyKey(story))), safePage + 11),
      8,
      new Set()
    );
    moreSections = [
  ...
  return {
    ...
    tickerStories: dedupeStories([
      hero,
      ...trending,
      ...moreSections.flatMap((section) => section.stories || []),
    ].filter(Boolean)),
    categoryMap,
    pageStories: filteredPageStories,
    totalPages,
...
function renderHomeTemplate(template = "", view = {}) {
  ...
  const tickerStories = dedupeStories((view.tickerStories || [hero, ...(view.trending || []), ...(view.moreSections || []).flatMap((section) => section.stories || [])]).filter(Boolean));
  ...
  output = output.replace(
    /<section class="home-top-layout">/,
    hasFullGrid
      ? '<section class="home-top-layout" hidden>'
      : '<section class="home-top-layout">'
  );
  output = replaceBlock(
    output,
    ...
    hasFullGrid
      ? [
        '<section class="hero-section" id="heroSection" aria-live="polite" hidden>',
        '<div class="hero-section__copy">',
        '<span class="hero-section__kicker" id="heroDeskChip">Live Desk</span>',
        '<h1 class="hero-section__title"><a id="headlineOfTheDayLink" href="/" target="_self" rel="noopener noreferrer">Loading headline...</a></h1>',
        '<p class="hero-section__summary" id="heroSummary">Fresh AI, tech, entertainment, sports, and business stories are loading.</p>',
        '<div class="hero-section__meta">',
        '<span class="hero-badge">Breaking</span>',
        '<span class="hero-section__author" id="heroAuthor">SunWire Desk</span>',
        '<span class="hero-section__time" id="headlineOfTheDayMeta">Waiting for fresh stories.</span>',
        '</div>',
        '</div>',
        '<div class="hero-section__media">',
        `<img id="heroImage" ${renderImageAttributes(SITE.socialImage, {
          width: 1600,
          height: 900,
          sizes: "(max-width: 1050px) 100vw, 66vw",
        })} alt="SunWire lead story" loading="eager" decoding="async" fetchpriority="high" />`,
        '</div>',
        '</section>',
      ].join("")
      : [
  ...
  output = output.replace(
    /<section class="trending-strip" aria-labelledby="trendingSectionTitle">[\s\S]*?<\/section>/,
    hasFullGrid
      ? [
        '<section class="trending-strip" aria-labelledby="trendingSectionTitle" hidden>',
        ...
        '</div>',
        '<span class="section-head__meta" id="trendingUpdatedAt">Updated just now</span>',
        '</div>',
        '</div>',
        '<div class="trending-strip__cards" id="trendingGrid" aria-live="polite"></div>',
        '</section>',
      ].join("")
      : [
        '<section class="trending-strip" aria-labelledby="trendingSectionTitle">',
        '<div class="section-head">',
        "<div>",
        '<p class="section-caption">Live momentum</p>',
        '<h2 class="section-title" id="trendingSectionTitle">Trending Now</h2>',
  ...
  output = output.replace(
    /<aside class="homepage-sidebar" id="homepageSidebar" aria-label="SunWire insights">/,
    hasFullGrid
      ? '<aside class="homepage-sidebar" id="homepageSidebar" aria-label="SunWire insights" hidden>'
      : '<aside class="homepage-sidebar" id="homepageSidebar" aria-label="SunWire insights">'
  );
  output = output.replace(
  ...
  output = output.replace(
    /<section class="category-zone" aria-labelledby="categoryNewsTitle">[\s\S]*?<\/section>/,
    hasFullGrid
      ? [
      ...
      : [
        '<section class="category-zone" aria-labelledby="categoryNewsTitle">',
        '<div class="section-head">',
        '<div>',
        '<p class="section-caption">Front page by desk</p>',
        `<h2 class="section-title" id="categoryNewsTitle">${escapeHtml(categoryHeading)}</h2>`,
  ...
  output = output.replace(
    ...
    hasFullGrid
      ? `<div class="news-card-grid news-card-grid--page" id="moreNewsGrid" aria-live="polite">${(view.fullGridStories || []).map((story) => renderNewsCard(story, "dense")).join("")}</div>`
      : `<div class="news-card-grid news-card-grid--homepage" id="moreNewsGrid" aria-live="polite">${((view.moreSections?.[0]?.stories) || []).map((story) => renderNewsCard(story, "dense")).join("")}</div>`,
  );
  output = output.replace(
  ...
  const preloadPayload = {
    ...
    categoryMap: toHomePreloadCategoryMap(view.categoryMap || {}),
  };

  return output.replace(
    /<script type="module" src="\/?app\.js\?v=[^"]+"><\/script>/,
    `<script>window.__SUNWIRE_HOME_DATA__=${safeJsonForInlineScript(preloadPayload)};</script><script type="module" src="/app.js?v=20260323-2"></script>`,
  );
}
...
function renderArticleTemplate(template = "", page = {}) {
  ...
  const relatedStories = Array.isArray(relatedSets.grid) ? relatedSets.grid.slice(0, 6) : [];
  ...
  output = output.replace(
    ...
    [
      ...
      "</section>",
      '<section class="article-block article-block--analysis">',
      '<div class="article-block__head"><p class="article-block__eyebrow">Key Takeaways</p><h2>Why It Matters</h2></div>',
      Array.isArray(article.keyPoints) && article.keyPoints.length
        ? `<ul class="bullet-list" id="keyPointsList">${renderBulletList(article.keyPoints, "No verified key facts available.")}</ul>`
        : `<div class="article-body" id="articleImpact">${renderParagraphGroup([article.indiaPulse || article.summary || "No additional verified details available."])}</div>`,
      "</section>",
      '<section class="article-block article-block--background">',
      '<div class="article-block__head"><p class="article-block__eyebrow">Context</p><h2>Background</h2></div>',
      renderBackgroundItems(background),
      "</section>",
      '<section class="article-block article-block--facts">',
      '<div class="article-block__head"><p class="article-block__eyebrow">Fast Facts</p><h2>Fact Sheet</h2></div>',
      renderFactSheet(Array.isArray(article.factSheet) ? article.factSheet : []),
      "</section>",
  ...
  output = output.replace(
    ...
    [
      ...
      '<div class="related-section__head"><p class="article-block__eyebrow">Internal Links</p><h2>Related Articles</h2></div>',
      `<div class="related-grid" id="relatedGrid">${renderRelatedGrid(relatedStories)}</div>`,
      "</section>",
    ].join(""),
  );
  ...
  return output.replace(
    /<script type="module" src="\/?article\.js\?v=20260321-1"><\/script>/,
    `<script>window.__SUNWIRE_ARTICLE_DATA__=${safeJsonForInlineScript(preloadPayload)};</script><script type="module" src="/article.js?v=20260321-1"></script>`,
  );
}
