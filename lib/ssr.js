...
function renderHomeTemplate(template = "", view = {}, runtimeConfig = {}) {
  ...
  const liveUpdates = Array.isArray(liveDeskStory?.activeLiveUpdates) ? liveDeskStory.activeLiveUpdates.slice(0, 4) : [];
  ...
  const liveMeta = !liveDeskStory
    ...
    : !liveUpdates.length
      ...
      : `${liveUpdates.length} live now${liveDeskTotal > liveUpdates.length ? ` • ${liveDeskTotal - liveUpdates.length} scheduled` : ""}`;
