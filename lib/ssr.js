...
function selectLiveDeskStory(storyPool = [], categoryMap = {}) {
  return dedupeStories([
    ...(Array.isArray(storyPool) ? storyPool : []),
    ...Object.values(categoryMap || {}).flatMap((stories) => Array.isArray(stories) ? stories : []),
  ])
    ...
    .sort((left, right) => {
      ...
      return new Date(storyTimestamp(right) || 0).getTime() - new Date(storyTimestamp(left) || 0).getTime();
    })[0] || null;
}
...
function buildHomeView({
  ...
  if (safeFilter !== "all") {
    ...
    return {
      hero: null,
      liveDeskStory: selectLiveDeskStory(focusedStories, categoryMap),
      trending: [],
      topSections: [],
      moreSections: [],
  ...
  if (safePage > 1) {
    ...
    return {
      hero: null,
      liveDeskStory: selectLiveDeskStory(fullGridStories, categoryMap),
      trending: [],
      topSections: [],
      moreSections: [],
      fullGridStories,
  ...
  const liveDeskStory = selectLiveDeskStory(storyPool, categoryMap);
...
function toHomePreloadStory(story = {}) {
  return {
    ...
    activeLiveUpdates: Array.isArray(story.activeLiveUpdates) ? story.activeLiveUpdates : [],
    liveUpdateTimeline: Array.isArray(story.liveUpdateTimeline) ? story.liveUpdateTimeline : [],
    nextLiveUpdateAt: story.nextLiveUpdateAt || "",
    liveUpdateCount: Number(story.liveUpdateCount || 0),
    activeLiveUpdateCount: Number(story.activeLiveUpdateCount || 0),
  };
}
...
function renderHomeTemplate(template = "", view = {}, runtimeConfig = {}) {
  ...
  const liveDeskStory = view.liveDeskStory || null;
  ...
  const activeLiveUpdates = Array.isArray(liveDeskStory?.activeLiveUpdates) ? [...liveDeskStory.activeLiveUpdates] : [];
  const liveTimeline = Array.isArray(liveDeskStory?.liveUpdateTimeline) ? [...liveDeskStory.liveUpdateTimeline] : [];
  const activeKeys = new Set(activeLiveUpdates.map((item) => `${cleanText(item?.text || "")}|${cleanText(item?.scheduledAt || "")}`));
  const liveUpdates = [...activeLiveUpdates, ...liveTimeline
    ...
    .slice(0, 3);
  const liveDeskTotal = Number(liveDeskStory?.liveUpdateCount || 0);
  const liveDeskNext = cleanText(liveDeskStory?.nextLiveUpdateAt || "");
  const liveNowCount = activeLiveUpdates.length;
  const liveMeta = !liveDeskStory
    ? "Waiting for live desk"
    : !liveUpdates.length
      ? (liveDeskTotal ? (liveDeskNext ? `Next line ${formatDateLabel(liveDeskNext)}` : "Quick lines are scheduled") : "No live timeline queued")
      : (liveNowCount
        ? `${liveNowCount} live now${liveDeskTotal > liveNowCount ? ` • ${liveDeskTotal - liveNowCount} scheduled` : ""}`
        : (liveDeskNext ? `Next line ${formatDateLabel(liveDeskNext)}` : "Quick lines are scheduled"));
  const liveListMarkup = !liveDeskStory
    ? "<li>Live updates will appear here after you push them from Watch All News.</li>"
    : !liveUpdates.length
      ? (liveDeskTotal
      ...
      : liveUpdates.map((update) => `<li><strong>${escapeHtml(cleanText(update?.text || "Live update"))}</strong><span>${escapeHtml(cleanText(update?.scheduledAt || "") ? formatDateLabel(update.scheduledAt) : "just now")}</span></li>`).join("");
  ...
  const topLayoutMarkup = [
    ...
    "</div>",
    "</div>",
      '<section class="hero-live-panel" id="heroLiveUpdatesPanel" aria-label="Live Updates">',
    '<div class="hero-live-panel__header">',
    "<div>",
    '<p class="hero-live-panel__eyebrow">Live Updates</p>',
    "</div>",
    `<span class="hero-live-panel__meta" id="heroLiveUpdatesMeta">${escapeHtml(liveMeta)}</span>`,
    "</div>",
    `<ul class="hero-live-panel__list" id="heroLiveUpdatesList">${liveListMarkup}</ul>`,
    "</section>",
    "</div>",
