...
function buildHomeView({
  ...
  const topSections = [];
  const recentPushes = fillSectionStories(
    sortStoriesByRecency(homepageFallbackPool.filter((story) => !used.has(storyKey(story)))),
    sortStoriesByRecency(storyPool.filter((story) => !used.has(storyKey(story)))),
    16,
    new Set()
  );
  const moreSections = [
...
function renderHomeTemplate(template = "", view = {}) {
  ...
  const topLayoutMarkup = [
    ...
    '</div>',
    '</article>',
    '<article class="sidebar-card sidebar-card--updates">',
    '<div class="sidebar-card__header"><span class="updates-icon">⚡</span> Latest Updates</div>',
    '<ul class="event-list" id="eventsList"><li>Loading events…</li></ul>',
    '</article>',
    '</aside>',
    '</div>',
  ...
  output = output.replace(
    ...
    `<section class="more-news-shell${cleanGridMode ? " more-news-shell--clean" : ""}" aria-labelledby="moreNewsTitle">`,
  );
  output = output.replace(
    /<h2 class="section-title" id="moreNewsTitle">[\s\S]*?<\/h2>/,
    `<h2 class="section-title" id="moreNewsTitle">${escapeHtml(hasFullGrid ? (view.pageTitle || `Page ${view.page} News Grid`) : "Recent Pushes")}</h2>`,
  );
  output = output.replace(
...
function renderArticleTemplate(template = "", page = {}) {
  ...
  output = output.replace(
    ...
    [
      ...
      "</section>",
      "</section>",
      (keyPoints.length || factSheet.length || indiaPulse || background.length || latestStories.length)
        ? [
          '<div class="article-secondary">',
          (keyPoints.length || factSheet.length)
            ? [
              '<div class="article-section-grid article-section-grid--facts">',
              keyPoints.length
                ? [
                  ...
                  `<div class="article-body">${renderParagraphGroup(keyPoints.map((point) => `${cleanText(point)}.`))}</div>`,
                  "</section>",
                ].join("")
                : "",
              factSheet.length
          ...
          latestStories.length
            ...
            : "",
          (indiaPulse || background.length)
            ? [
              '<div class="article-section-grid article-section-grid--context">',
              indiaPulse
                ? [
                  ...
                  `<div class="article-body"><p>${escapeHtml(indiaPulse)}</p></div>`,
                  "</section>",
                ].join("")
                : "",
              background.length
  ...
  return output.replace(
    ...
    `<script>window.__SUNWIRE_ARTICLE_DATA__=${safeJsonForInlineScript(preloadPayload)};</script><script type="module" src="/article.js?v=20260326-16"></script>`,
  );
}
